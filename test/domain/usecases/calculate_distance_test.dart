...
void main() {
  group('CalculateDistanceUseCase', () {
    ...
    test('debe tener precisión del ±2% en distancias >10 km', () {
      // Arrange - Valencia a Sevilla (~540 km)
    ...
    test('debe usar la fórmula de Haversine correctamente', () {
      // Arrange - Bilbao a Zaragoza (~246 km real)
      const bilbaoLat = 43.2630;
      const bilbaoLon = -2.9350;
      const zaragozaLat = 41.6488;
      const zaragozaLon = -0.8891;
      ...
      // Assert - Verificar que usa Haversine (más precisa que distancia euclidiana)
      expect(distance, greaterThan(240));
      expect(distance, lessThan(250));
    });

    test('debe manejar diferencias muy pequeñas de coordenadas', () {
      // Arrange - Dos puntos muy cercanos (~10 metros)
      const lat1 = 40.4168;
      const lon1 = -3.7038;
      const lat2 = 40.4169; // ~100 metros al norte
      const lon2 = -3.7038;
      ...
      // Assert - Debe ser muy pequeña pero > 0
      expect(distance, greaterThan(0.0));
      expect(distance, lessThan(0.2)); // Menos de 200 metros
    });

    test('debe producir resultados consistentes con múltiples llamadas', () {
      // Arrange
      const lat1 = 40.4168;
      const lon1 = -3.7038;
      const lat2 = 41.3851;
      ...
      // Act - Llamar 3 veces
      final distance1 = useCase(lat1: lat1, lon1: lon1, lat2: lat2, lon2: lon2);
      final distance2 = useCase(lat1: lat1, lon1: lon1, lat2: lat2, lon2: lon2);
      final distance3 = useCase(lat1: lat1, lon1: lon1, lat2: lat2, lon2: lon2);
    ...
    test('debe manejar longitudes que cruzan el meridiano de Greenwich', () {
      // Arrange - Londres (lon negativo) a París (lon positivo)

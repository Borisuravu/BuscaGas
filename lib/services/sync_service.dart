...
class SyncService {
  ...
  void startPeriodicSync() {
    ...
    debugPrint(
        '[SyncService] Sincronización periódica iniciada (intervalo: ${syncInterval.inMinutes} min)');
  }
  ...
  void stopPeriodicSync() {
    _syncTimer?.cancel();
    _syncTimer = null;
    debugPrint('[SyncService] Sincronización periódica detenida');
  }
  ...
  Future<void> performSync() async {
    ...
    if (_isSyncing) {
      debugPrint('[SyncService] Sincronización ya en progreso, ignorando');
      return;
    }

    _isSyncing = true;
    debugPrint('[SyncService] Iniciando sincronización...');
    ...
    try {
      ...
      if (!hasConnection) {
        debugPrint(
            '[SyncService] Sin conexión a internet, cancelando sincronización');
      ...
      debugPrint('[SyncService] Descargando datos frescos desde API...');
      List<GasStation> freshData = await _repository.fetchRemoteStations();
      debugPrint('[SyncService] Descargados ${freshData.length} registros');

      // 3. Obtener datos de caché
      List<GasStation> cachedData = await _repository.getCachedStations();
      debugPrint('[SyncService] Caché actual: ${cachedData.length} registros');

      // 4. Comparar datos
      bool hasChanges = _hasDataChanged(freshData, cachedData);

      if (hasChanges) {
        debugPrint('[SyncService] Cambios detectados, actualizando caché...');
        ...
        debugPrint('[SyncService] Caché actualizada exitosamente');

        // 7. Notificar que hay datos nuevos
        onDataUpdated?.call();
      } else {
        debugPrint('[SyncService] No se detectaron cambios en los datos');
        _lastSyncTime = DateTime.now();
      }
      ...
      debugPrint(
          '[SyncService] Sincronización completada exitosamente a las ${DateTime.now()}');
    } catch (e) {
      debugPrint('[SyncService] Error durante sincronización: $e');
  ...
  Future<bool> _hasInternetConnection() async {
    try {
      ...
      debugPrint('[SyncService] Error verificando conectividad: $e');
      return false;
    }
  }
  ...
  bool _hasDataChanged(List<GasStation> fresh, List<GasStation> cached) {
    ...
    if (fresh.length != cached.length) {
      debugPrint(
          '[SyncService] Cambio detectado: diferente cantidad de registros');
      return true;
    }
    ...
    for (int i = 0; i < sampleSize; i++) {
      ...
      if (fresh[i].prices.length != cached[i].prices.length) {
        debugPrint(
            '[SyncService] Cambio detectado en precios de estación ${fresh[i].id}');
        return true;
      }
      ...
      for (int j = 0; j < fresh[i].prices.length; j++) {
        if (fresh[i].prices[j].value != cached[i].prices[j].value) {
          debugPrint('[SyncService] Cambio detectado en precio de combustible');
          return true;
        }
      }
    }
